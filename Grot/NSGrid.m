...
CGPoint const NSGridInvalidPoint = (CGPoint){ -1.0, -1.0 };

@implementation NSGrid
...
{
    CGPoint point = NSGridInvalidPoint;
...
- (NSGrid*)gridBySubtractingGrid:(NSGrid*)grid
{
    ...
    NSGrid* resultGrid = [NSGrid.alloc initWithSize:self.size];
...
- (NSUInteger)hash
{
    NSUInteger aggregate = NSPair_Szudzik(self.size.width, self.size.height);
    
    for (NSObject* item in _items)
        aggregate = NSPair_Szudzik(aggregate, item.hash);
