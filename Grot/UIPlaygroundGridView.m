...
#pragma mark - Size and color observation
...
{
    NSGameGrid* grid = self.dataSource.gameGridForPlaygroundGridView;
    NSArrowField* arrowField = [grid objectAtPoint:point];
...
- (void)prepareArrowViewsForReaction:(NSFieldReaction*)reaction
{
    ...
    {
        NSFieldTransition* transition = transitions[i];
        NSArrowField* field = transition.arrowField;
...
#pragma mark - Grid view animations
...
- (void)reloadMissingFieldsAnimated:(BOOL)animated
{
    ...
    for (NSArrowField* field in orphanFields)
    {
...
- (NSArray*)arrowFieldsWithoutArrowView
{
    ...
    for (NSArrowField* field in grid.allObjects)
    {
        UIArrowView* view = [self arrowViewForArrowField:field];
...
- (NSArray*)arrowViewsForArrowFields:(NSArray*)arrowFields
{
    ...
    for (NSArrowField* field in arrowFields)
    {
        UIArrowView* view = [self arrowViewForArrowField:field];
